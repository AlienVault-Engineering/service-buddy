...
class InitTestCase(ParentTestCase):
    ...
    def test_service_definition_generation(self):
        ...
        application_map = loader.load_service_definitions(service_directory,code_directory=self.temp_dir)
        init = Initializer(self.vcs, self.temp_dir, self.service_templates_test,skip_git_creation=True)
        init.initialize_services(application_map)
